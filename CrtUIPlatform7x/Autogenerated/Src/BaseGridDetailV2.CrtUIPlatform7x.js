...
define("BaseGridDetailV2", ["BaseGridDetailV2Resources", "ConfigurationEnums", "ChangeLogUtilities", "RightUtilities",
	...
	return {
		...
		methods: {
			 ...
			 * @overridden
			 */
			getEditPages: function(operation) {
				let editPages = this.callParent();
				const page = editPages?.first();
				const pageOperation = page?.$Operation;
				const isPageForAddOperation = pageOperation === Terrasoft.ConfigurationEnums.CardOperation.ADD;
				const isEditOrCopyOperation = operation === Terrasoft.ConfigurationEnums.CardOperation.EDIT ||
					operation === Terrasoft.ConfigurationEnums.CardOperation.COPY;
				const needGetPagesByOperation = isEditOrCopyOperation && isPageForAddOperation;
				if (needGetPagesByOperation) {
					editPages = this.getEditPagesCollection(operation);
				}
				return editPages;
			},
