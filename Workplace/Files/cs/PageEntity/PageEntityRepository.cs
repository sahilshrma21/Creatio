namespace Terrasoft.Configuration.PageEntity
{
	...
	public class PageEntityRepository : IPageEntityRepository
	{
		...
		private IEnumerable<PageEntity> GetRelatedPages() {
			var pagesManager = ClassFactory.Get<IRelatedPagesManager>();
			var relatedPages = pagesManager.GetRelatedPages(nameof(EntitySchemaManager)) ?? new List<SchemaRelatedPage>();
			return relatedPages.SelectMany(srp => srp.RelatedPages.Select(rp => PageEntity.CreatePageInstance(srp, rp)));
		}
		...
		public IEnumerable<PageEntity> GetAll() {
			...
			var angularPages = GetRelatedPages();
			pages.AddRangeIfNotExists(angularPages);
			return pages;
		}
