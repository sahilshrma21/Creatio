namespace Terrasoft.Configuration.WorkplaceSectionAccess
{
	...
	public abstract class BaseWorkplaceSectionAccessManager : IBaseWorkplaceSectionAccessManager
	{
		...
		private Dictionary<Guid, List<Section>> GetSectionsInWorkplaces() {
			...
			IEnumerable<Section> allSections = SectionRepository.GetAll();
		...
		protected IEnumerable<Workplace> GetAllAllowedWorkplacesInternal(Guid userId) {
			ICollection<Workplace> allWorkplaces = WorkplaceRepository.GetAll()
				.Where(workplace => workplace.GetIsAllowedForUser(userId)).ToList();
			if (!GetUserHasLimits()) {
				return SortWorkplaces(allWorkplaces);
			}
			return SortWorkplaces(allWorkplaces.Where(workplace => AllowedWorkplaceTypes.Contains(workplace.Type)))
				.Take(1);
		}
