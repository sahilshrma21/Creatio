namespace IntegrationV2.Files.cs.Failover
{
	...
	public abstract class BaseFailoverJob<T>: IJobExecutor
	{
		...
		private readonly Lazy<IFeatureUtilities> _featureUtilities = new Lazy<IFeatureUtilities>(() => ClassFactory.Get<IFeatureUtilities>());

		protected IFeatureUtilities FeatureUtilities {
			get {
				return _featureUtilities.Value;
			}
		}
		...
		protected bool GetIsFeatureEnabledForAnyUser(UserConnection userConnection, string code) {
			return FeatureUtilities.GetIsFeatureEnabledForAnyUser(userConnection, code);
		}
