namespace Terrasoft.Configuration
{
	...
	public class ExchangeListenerManager : IExchangeListenerManager
	{
		...
		private SynchronizationCredentials GetSynchronizationCredentials(string mailboxName) {
			var mailbox = _mailboxService.GetMailboxBySenderEmailAddress(mailboxName.Trim());
			if (mailbox == null) {
		...
		private SynchronizationCredentials GetSynchronizationCredentials(Mailbox mailbox) {
			...
			credentials.BpmEndpoint = GetApplicationEndpoint();
			return credentials;
		}

		/// <summary>
		/// Get application enpoint.
		/// </summary>
		/// <returns>Application endpoint.</returns>
		private string GetApplicationEndpoint() {
			...
			return utils.GetBpmEndpointUrl();
		}
		...
		/// </summary>
		/// <returns>Serialized data array.</returns>
		private byte[] Serialize(object obj) {
			return Encoding.UTF8.GetBytes(Json.Serialize(obj));
		}
		...
		public void StopListener(Guid mailboxId) {
			...
			byte[] data = Serialize(new[] { mailboxId });
			var mailbox = _mailboxService.GetMailbox(mailboxId);
			TryDoListenerAction(mailbox.SenderEmailAddress, () => {
		...
		public void UpdateListener(Guid mailboxId, string senderEmailAddress) {
			_log.DebugFormat($"mailbox {mailboxId} UpdateListener started");
			try {
				byte[] data = GetConnectionParams(mailboxId);
				_log.DebugFormat($"mailbox {mailboxId} ExecuteListenerAction started");
				TryDoListenerAction(senderEmailAddress, () => {
					ExecuteListenerAction(data, ExchangeListenerActions.Update);
				});
				_log.DebugFormat($"mailbox {mailboxId} ExecuteListenerAction ended");
			} catch (Exception ex) {
				_log.Error($"mailbox {mailboxId} UpdateListener error", ex);
				throw;
			}
			_log.DebugFormat($"mailbox {mailboxId} UpdateListener ended");
		}
		...
		/// <inheritdoc cref="IExchangeListenerManager.GetIsApplicationAvaliable"/>
		public bool GetIsApplicationAvailable() {
			if (GetIsFeatureEnabled("OldEmailIntegration")) {
				return false;
			}
			HttpWebResponse response = null;
			try {
				var data = Serialize(GetApplicationEndpoint());
				var serviceUri = ExchangeListenerActions.GetAvailableUrl(UserConnection, ExchangeListenerActions.ApplicationAvailable);
				var responseString = ExecuteAction(data, serviceUri);
				return responseString.Equals(ExchangeListenerActions.ApplicationAvailableResponse, StringComparison.CurrentCultureIgnoreCase);
			} finally {
				response?.Close();
			}
		}
		...
		public Dictionary<Guid, string> GetSubscriptionsStatuses(Guid[] mailboxIds) {
			...
			byte[] data = Serialize(mailboxIds);
