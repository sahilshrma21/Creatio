namespace IntegrationV2.Files.cs.Domains.MeetingDomain.Repository
{
	...
	public class ParticipantRepository : IParticipantRepository
	{
		...
		private Dictionary<Guid, string> GetContactsByEmails(HashSet<string> emails) {
			...
			using (DBExecutor dbExecutor = _userConnection.EnsureDBConnection()) {
				using (var reader = select.ExecuteReader(dbExecutor)) {
					while (reader.Read()) {
						contacts[reader.GetColumnValue<Guid>("ContactId")] = reader.GetColumnValue<string>("EmailAddress");
					}
				}
			}
			return contacts;
