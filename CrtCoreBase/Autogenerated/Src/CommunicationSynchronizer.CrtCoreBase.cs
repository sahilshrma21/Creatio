namespace Terrasoft.Configuration
{
	...
	public class CommunicationSynchronizer {
		...
		private Entity GetPrimaryCommunicationEntityFromMultiple(EntityCollection communicationEntities, 
			...
			var duplicatePrimaryCommunicationEntitiesIds = communicationEntities.Where(c => (string)c.GetColumnValue(NumberColumnName) != comparisonColumnValue)
					.Select(c => (Guid)c.GetColumnValue("Id"));
			...
			update.Execute();
			return communicationEntities.First(c => (string)c.GetColumnValue(NumberColumnName) == comparisonColumnValue);
		}
