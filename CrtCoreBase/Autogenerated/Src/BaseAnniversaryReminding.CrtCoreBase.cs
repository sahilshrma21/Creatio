namespace Terrasoft.Configuration
{
	...
	public abstract class BaseAnniversaryReminding
	{
		...
		private bool CanReceiverReadConnectedEntity(Entity reminding, string schemaName) {
			Dictionary<string, object> conditions = new Dictionary<string, object> {
				{ "Contact", reminding.GetTypedColumnValue<Guid>("ContactId") },
				{ "Active", true }
			};
			...
			SchemaRecordRightLevels rightsLevel = UserConnection.DBSecurityEngine.GetEntitySchemaRecordRightLevel(
				sysAdminUnit.GetTypedColumnValue<Guid>("Id"), schemaName, reminding.GetTypedColumnValue<Guid>("SubjectId"));
		...
		private void GenerateRemindings(Select select, string anniversarySchemaName) {
			...
			var remindings = new List<Entity>();
